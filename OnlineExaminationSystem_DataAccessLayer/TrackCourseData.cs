...
namespace OnlineExaminationSystem_DataAccessLayer
{
    public class TrackCourseData
    {
        ...
        public static bool UpdateTrackCourseInfo(int? trackCourseID, int trackID, int courseID)
        {
            int rowsAffected = 0;
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(DataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("SP_TrackCourses_UpdateTrackCourseInfo", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@TrackCourseID", trackCourseID);
                        command.Parameters.AddWithValue("@TrackID", trackID);
                        command.Parameters.AddWithValue("@CourseID", courseID);
